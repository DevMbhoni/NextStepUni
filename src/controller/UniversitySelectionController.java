...
public class UniversitySelectionController extends BaseController implements Initializable {
    ...
    private Node createUniversityCard(University uni) {
        ...
        Button profileButton = new Button("Select");
        ...
        profileButton.setOnAction(event -> {
            ...
            System.out.println("Selected University: " + uni.getUniName() + " (ID: " + uni.getUniversityID() + ")");

            SceneManager.switchTo("/view/view_uni_review.fxml",
                    (ViewUniReviewController controller) -> controller.initData(uni)
            );
        });

        card.getChildren().addAll(logoView, nameLabel, profileButton);

        final DropShadow hoverShadow = new DropShadow(20, Color.rgb(213, 30, 30, 0.5));
        final DropShadow defaultShadow = new DropShadow(10, Color.rgb(0, 0, 0, 0.1));
        card.setEffect(defaultShadow);
        card.setOnMouseEntered(e -> card.setEffect(hoverShadow));
        card.setOnMouseExited(e -> card.setEffect(defaultShadow));
